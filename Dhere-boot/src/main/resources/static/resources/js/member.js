...
$(function() {
	...
	$("#authenNumBtn").click(function() {
		if ($("#authenNum").val() == code2) { // 위에서 저장한값을 ㅣ교함
			$("#label3").css("color", "white").text("인증에 성공하였습니다.");
			$("#authenNumCheck").val("true");
			console.log($("#authenNumCheck").val());
		} else {
			$("#label3").css("color", "red").text("인증에 실패하였습니다.");
		}
	});
	...
	$("#btnOverlapNickName").on("click", function() {
		...
		if (nickname == '' || nickname.length == 0) {
			$("#label4").css("color", "red").text("공백은 닉네임으로 사용할 수 없습니다.");
			return false;
		}
		...
		$.ajax({
			...
			success: function(resData) {
				...
				if (resData.result) {
					...
					$("#label4").css("color", "white").text("사용 가능한 닉네임 입니다.");
					...
					$("#label4").css("color", "red").text("중복된 닉네임 입니다.");
				}
			},
			"error": function(xhr, statusText, err) {
				console.error("error …");
